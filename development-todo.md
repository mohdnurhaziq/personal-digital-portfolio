...
- [x] Reordering — up/down buttons persisting a new `sort_order`. Chose buttons over drag-and-drop: they work with a keyboard and on touch, which drag alone does not.
  ...
  Accessibility hardening now gives every control an item-specific accessible name and position, announces successful or failed moves through a polite live region, restores focus to an enabled control after the Inertia request, and rolls an optimistic move back when the server rejects it. The same tested reorder helper drives record lists and project screenshots.

  **Browser validation:** Enter and Space both persist a move, announce the new position, and keep focus on an enabled control at the top boundary. A forced local validation rejection restored the original order, announced the failure, and retained focus. At 390 px and 320 px the page has no body-level overflow, the table scrolls within its own container, and the 28×26 px controls remain above WCAG 2.2's 24×24 px minimum target size. The local database had no saved project screenshots, so that surface was covered by the shared JavaScript tests and Laravel media-reorder tests rather than a live browser interaction. An actual VoiceOver/NVDA pass and physical-phone test are still outstanding.
...
- [x] Lighthouse audit — run against the **production** stack, since the dev server ships unminified modules and measuring it would be meaningless. Desktop is now **100 / 100 / 100 / 100** on all three pages. Three real defects had to be fixed to get there:

  - **Nothing was compressed.** nginx had no `gzip`, which Lighthouse costed at ~800ms on the programmer page — the largest single win available, and it needed no application change.
  ...
  **Mobile 3D profile implemented:** touch / small screens (`max-width: 720px` or coarse pointer) now run the same WebGL scene with a lightweight profile: 650 particles, 1x DPR, antialiasing disabled, and a low-power GPU preference. Desktop retains 1,800 particles and up to 2x DPR. Reduced-motion visitors still receive the CSS radial gradient.
